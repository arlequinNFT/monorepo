...
import { setColor } from './ground-light-color.reducer';

const GroundLightColor = () => {
  const unityContext = useAppSelector((state) => state.painter.unityContext);

  const dispatch = useAppDispatch();
  const color = useAppSelector((state) => state.groundLightColor.color);

  const updateColor = useDebouncedCallback((value: string) => {
    unityContext?.send('HudManager', 'SetGroundLightColor', value);
...
export default GroundLightColor;
